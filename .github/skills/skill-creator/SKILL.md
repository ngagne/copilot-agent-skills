...
### Step 0: Install an isolated candidate copy into the user skills directory

Before running evals, copy the candidate skill into Copilot's personal skills directory using a unique GUID-suffixed folder name so existing skills are never overwritten.

- macOS/Linux: `~/.copilot/skills/<frontmatter-name>-<guid>/`
- Windows: `%USERPROFILE%\.copilot\skills\<frontmatter-name>-<guid>\`

Then update the copied `SKILL.md` frontmatter so `name` exactly matches that copied folder name. This keeps the skill callable from Copilot CLI and preserves the required `name` ↔ parent-directory match.

Always evaluate using this isolated installed copy, not the source folder, and never reuse a previous GUID folder.
...
Treat description optimization as a normal phase. The bundled automation now uses isolated helper-agent sessions in the current runtime.
...
This automation currently depends on the local `copilot` CLI because the bundled scripts spawn isolated helper-agent sessions and temporarily install the candidate skill into Copilot's personal skill directory using the same GUID-isolated naming rule (`<frontmatter-name>-<guid>` with matching copied frontmatter `name`) to measure trigger behavior. Run it only when that runtime is available. Otherwise, prepare the eval set and explain the requirement instead of pretending the automated optimization ran.

`scripts.run_loop` may open a live auto-refreshing HTML report before the loop finishes. Treat that page as internal progress only. Do not show it to the user until the loop exits and the final HTML is written. Prefer `--results-dir <dir>` so you can hand off the completed `report.html`, or regenerate a final static report from `results.json` with `python -m scripts.generate_report <results.json> -o <report.html>` before sharing it.
...
When helper agents are available, use them for parallel candidate/baseline runs, grading, blind comparison, and benchmark analysis.
